...
public:
    ...
    constexpr explicit Tuple(T &&v, Rest &&... rest)
        : head(std::forward<T>(v)), tail(std::forward<Rest>(rest)...) {
    }
    ...
    constexpr void set(auto &&val) & {
        if constexpr (I == 0) {
            head = std::forward<decltype(val)>(val);
        } else {
            tail.template set<I - 1>(std::forward<decltype(val)>(val));
        }
    }
    ...
    constexpr auto &front() & { return head; }
    constexpr const auto &front() const & { return head; }
    constexpr auto &&front() && { return std::move(head); }
    ...
    constexpr auto pop_back() const & {
        return pop_back_impl(std::make_index_sequence<sizeof...(Rest)>{});
    }

    constexpr auto pop_back() && {
        return std::move(*this).pop_back_impl(std::make_index_sequence<sizeof...(Rest)>{});
    }
    ...
    constexpr auto concat(Tuple<Us...> &&other) && {
        return std::move(*this).concat_impl(std::move(other), std::index_sequence_for<T, Rest...>{},
...
private:
    ...
    constexpr auto pop_back_impl(std::index_sequence<Is...>) const & {
        return Tuple<T, std::tuple_element_t<Is, Tuple<Rest...> >...>{head, tail.template get<Is>()...};
    }

    template<std::size_t... Is>
    constexpr auto pop_back_impl(std::index_sequence<Is...>) && {
        return Tuple<T, std::tuple_element_t<Is, Tuple<Rest...> >...>{
            std::move(head), std::move(tail).template get<Is>()...
        };
    }
    ...
    constexpr auto map_impl(F &&f, std::index_sequence<Is...>) & {
        return Tuple<decltype(f(get<Is>()))...>{f(get<Is>())...};
    }
    ...
    template<typename F, std::size_t... Is>
    constexpr auto map_impl(F &&f, std::index_sequence<Is...>) && {
        return Tuple<decltype(f(std::move(*this).template get<Is>()))...>{f(std::move(*this).template get<Is>())...};
    }

    template<typename F, std::size_t... Is>
    constexpr void map_mut_impl(F &&f, std::index_sequence<Is...>) & {
        ((get<Is>() = f(get<Is>())), ...);
    }
    ...
    template<typename F, std::size_t... Is>
    constexpr void apply_mut_impl(F &&f, std::index_sequence<Is...>) & {
    ...
    constexpr auto concat_impl(const Tuple<Us...> &other, std::index_sequence<Is...>,
    ...
    constexpr auto reverse_impl(std::index_sequence<Is...>) && {
        return Tuple<std::tuple_element_t<sizeof...(Is) - 1 - Is, Tuple>...>{
            std::move(*this).template get<sizeof...(Is) - 1 - Is>()...
        };
    }

    template<std::size_t... Is>
    constexpr void swap_impl(Tuple &other, std::index_sequence<Is...>) & {
        (std::swap(get<Is>(), other.template get<Is>()), ...);
    }
    ...
    static constexpr auto fold_left_helper(F &&f, Acc &&acc, U &&u, Us &&... us) {
        if constexpr (sizeof...(Us) == 0) {
            ...
            return fold_left_helper(std::forward<F>(f), std::forward<F>(f)(std::forward<Acc>(acc), std::forward<U>(u)),
                                    std::forward<Us>(us)...);
        }
    }
    ...
    constexpr auto filter_impl(Pred &&pred, std::index_sequence<Is...>) const & {
        return filter_concat(std::forward<Pred>(pred), get<Is>()...);
    }

    template<typename Pred>
    static constexpr auto filter_concat(Pred &&) {
        return Tuple<>{};
    }

    template<typename Pred, typename U, typename... Us>
    static constexpr auto filter_concat(Pred &&pred, U &&u, Us &&... us) {
        if constexpr (sizeof...(Us) == 0) {
            if (pred(u)) {
            ...
            if (pred(u)) {
                auto rest = filter_concat(std::forward<Pred>(pred), std::forward<Us>(us)...);
                return Tuple<std::decay_t<U> >{std::forward<U>(u)}.concat(std::move(rest));
            } else {
                return filter_concat(std::forward<Pred>(pred), std::forward<Us>(us)...);
            }
        }
    }
    ...
    template<std::size_t N, std::size_t... Is>
    constexpr void fill_impl(const auto &val, std::index_sequence<Is...>) & {
        ((get<Is>() = val), ...);
    }
};
